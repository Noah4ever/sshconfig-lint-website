...
export const copy: Record<Locale, Copy> = {
  en: {
    ...
    allRules: 'View all 25 rule guides',
    rules: [
      ...
      { code: 'NEGATED_HOST', title: 'Negated-only hosts', text: 'Finds Host blocks that can never produce a positive match.' },
      { code: 'PROXY_CONFLICT', title: 'Conflicting proxy methods', text: 'Shows when ProxyCommand and ProxyJump compete in one scope.' },
      { code: 'LOCAL_COMMAND_DISABLED', title: 'Ignored local commands', text: 'Warns when LocalCommand is present but not enabled.' },
      { code: 'INVALID_TOKEN', title: 'Invalid percent tokens', text: 'Checks placeholders against each directive’s supported tokens.' },
    ],
    ...
    browserItems: ['No installation', 'No upload', '17 content-based rules', 'Works on phones and desktops'],
    ...
    findingMessages: {
      ...
      invalidIntegerRange: "{directive} has the invalid value '{target}'. Expected an integer from {min} to {max}.",
      ...
      negatedOnlyHost: 'Host {patterns} has no positive pattern and can never match.',
      proxyConflict: '{ignoredDirective} has no effect because {firstDirective} was already set on line {first}.',
      localCommandDisabled: 'LocalCommand is ignored because PermitLocalCommand is not enabled.',
      invalidPercentToken: '{token} is not supported by {directive}.',
      unknownDirective: '{directive} is not a known OpenSSH client option.',
      deprecatedOption: '{directive} is deprecated or obsolete.',
      controlPersistUnused: 'ControlPersist cannot take effect without ControlMaster.',
      updateHostKeysPersist: 'UpdateHostKeys ask cannot prompt while ControlPersist is active.',
      invalidSyntax: '{directive} has a missing argument, an empty argument, or unbalanced quotes.',
      invalidMatchCondition: 'The Match condition near {criterion} is invalid.',
    },
    findingHints: {
      ...
      invalidIntegerRange: 'Use a whole number inside the stated range.',
      ...
      negatedOnlyHost: 'Add a positive pattern such as * or remove the block.',
      proxyConflict: 'Keep the one proxy mechanism that should apply in this scope.',
      localCommandDisabled: 'Add PermitLocalCommand yes in an applicable scope or remove LocalCommand.',
      invalidPercentToken: 'Use a token supported by this directive, or write %% for a literal percent sign.',
      unknownDirective: 'Check the spelling or guard a platform-specific option with IgnoreUnknown.',
      deprecatedOption: 'Remove the option or replace it with its current OpenSSH equivalent.',
      controlPersistUnused: 'Enable ControlMaster in an applicable scope or remove ControlPersist.',
      updateHostKeysPersist: 'Use UpdateHostKeys yes or disable ControlPersist in this scope.',
      invalidSyntax: 'Add the required value and close all single or double quotes.',
      invalidMatchCondition: 'Use a supported Match criterion and provide its required value.',
    },
  },
  de: {
    ...
    allRules: 'Alle 25 Regelbeschreibungen ansehen',
    rules: [
      ...
      { code: 'NEGATED_HOST', title: 'Nur negierte Hosts', text: 'Findet Host-Blöcke, die nie positiv passen können.' },
      { code: 'PROXY_CONFLICT', title: 'Widersprüchliche Proxy-Wege', text: 'Zeigt, wenn ProxyCommand und ProxyJump im selben Bereich konkurrieren.' },
      { code: 'LOCAL_COMMAND_DISABLED', title: 'Ignorierte lokale Befehle', text: 'Warnt, wenn LocalCommand vorhanden, aber nicht freigeschaltet ist.' },
      { code: 'INVALID_TOKEN', title: 'Ungültige Prozent-Tokens', text: 'Prüft Platzhalter gegen die erlaubten Tokens jeder Direktive.' },
    ],
    ...
    browserColumn: 'In diesem Browser', browserItems: ['Keine Installation', 'Kein Upload', '17 inhaltsbasierte Regeln', 'Auf Smartphone und Desktop nutzbar'],
    ...
    findingMessages: {
      ...
      invalidIntegerRange: "{directive} hat den ungültigen Wert '{target}'. Erwartet wird eine ganze Zahl von {min} bis {max}.",
      ...
      negatedOnlyHost: 'Host {patterns} enthält kein positives Muster und kann nie passen.',
      proxyConflict: '{ignoredDirective} ist wirkungslos, weil {firstDirective} bereits in Zeile {first} gesetzt wurde.',
      localCommandDisabled: 'LocalCommand wird ignoriert, weil PermitLocalCommand nicht aktiviert ist.',
      invalidPercentToken: '{token} wird von {directive} nicht unterstützt.',
      unknownDirective: '{directive} ist keine bekannte OpenSSH-Client-Option.',
      deprecatedOption: '{directive} ist veraltet oder wird nicht mehr unterstützt.',
      controlPersistUnused: 'ControlPersist kann ohne ControlMaster nicht wirken.',
      updateHostKeysPersist: 'UpdateHostKeys ask kann bei aktivem ControlPersist nicht nachfragen.',
      invalidSyntax: '{directive} hat einen fehlenden oder leeren Wert oder nicht geschlossene Anführungszeichen.',
      invalidMatchCondition: 'Die Match-Bedingung bei {criterion} ist ungültig.',
    },
    findingHints: {
      ...
      invalidIntegerRange: 'Verwende eine ganze Zahl innerhalb des angegebenen Bereichs.',
      ...
      negatedOnlyHost: 'Füge ein positives Muster wie * hinzu oder entferne den Block.',
      proxyConflict: 'Behalte den einen Proxy-Weg, der in diesem Bereich gelten soll.',
      localCommandDisabled: 'Füge PermitLocalCommand yes in einem passenden Bereich hinzu oder entferne LocalCommand.',
      invalidPercentToken: 'Nutze ein unterstütztes Token oder %% für ein wörtliches Prozentzeichen.',
      unknownDirective: 'Prüfe die Schreibweise oder sichere eine plattformspezifische Option mit IgnoreUnknown ab.',
      deprecatedOption: 'Entferne die Option oder ersetze sie durch ihr aktuelles OpenSSH-Pendant.',
      controlPersistUnused: 'Aktiviere ControlMaster in einem passenden Bereich oder entferne ControlPersist.',
      updateHostKeysPersist: 'Nutze UpdateHostKeys yes oder deaktiviere ControlPersist in diesem Bereich.',
      invalidSyntax: 'Füge den benötigten Wert hinzu und schließe alle Anführungszeichen.',
      invalidMatchCondition: 'Nutze eine unterstützte Match-Bedingung mit dem benötigten Wert.',
    },
  },
  fr: {
    ...
    allRules: 'Voir les 25 guides de règles',
    rules: [
      ...
      { code: 'NEGATED_HOST', title: 'Hôtes uniquement négatifs', text: 'Repère les blocs Host qui ne peuvent jamais correspondre positivement.' },
      { code: 'PROXY_CONFLICT', title: 'Méthodes proxy en conflit', text: 'Montre quand ProxyCommand et ProxyJump se concurrencent dans une portée.' },
      { code: 'LOCAL_COMMAND_DISABLED', title: 'Commandes locales ignorées', text: 'Avertit lorsque LocalCommand est présent sans être activé.' },
      { code: 'INVALID_TOKEN', title: 'Jetons de pourcentage invalides', text: 'Vérifie les variables autorisées pour chaque directive.' },
    ],
    ...
    browserColumn: 'Dans ce navigateur', browserItems: ['Aucune installation', 'Aucun envoi', '17 règles textuelles', 'Mobile et ordinateur'],
    ...
    findingMessages: {
      ...
      invalidIntegerRange: "{directive} contient la valeur invalide '{target}'. Un entier de {min} à {max} est attendu.",
      ...
      negatedOnlyHost: 'Host {patterns} ne contient aucun motif positif et ne peut jamais correspondre.',
      proxyConflict: '{ignoredDirective} est sans effet car {firstDirective} est déjà défini à la ligne {first}.',
      localCommandDisabled: 'LocalCommand est ignoré car PermitLocalCommand n’est pas activé.',
      invalidPercentToken: '{token} n’est pas accepté par {directive}.',
      unknownDirective: '{directive} n’est pas une option client OpenSSH connue.',
      deprecatedOption: '{directive} est obsolète ou n’est plus pris en charge.',
      controlPersistUnused: 'ControlPersist est sans effet sans ControlMaster.',
      updateHostKeysPersist: 'UpdateHostKeys ask ne peut pas demander confirmation avec ControlPersist actif.',
      invalidSyntax: '{directive} contient un argument manquant ou vide, ou des guillemets non fermés.',
      invalidMatchCondition: 'La condition Match près de {criterion} est invalide.',
    },
    findingHints: {
      ...
      invalidIntegerRange: 'Utilisez un entier dans la plage indiquée.',
      ...
      negatedOnlyHost: 'Ajoutez un motif positif comme * ou supprimez le bloc.',
      proxyConflict: 'Conservez la méthode proxy qui doit réellement s’appliquer dans cette portée.',
      localCommandDisabled: 'Ajoutez PermitLocalCommand yes dans une portée applicable ou supprimez LocalCommand.',
      invalidPercentToken: 'Utilisez un jeton accepté ou %% pour un signe de pourcentage littéral.',
      unknownDirective: 'Vérifiez l’orthographe ou protégez une option spécifique à une plateforme avec IgnoreUnknown.',
      deprecatedOption: 'Supprimez l’option ou remplacez-la par son équivalent OpenSSH actuel.',
      controlPersistUnused: 'Activez ControlMaster dans une portée applicable ou supprimez ControlPersist.',
      updateHostKeysPersist: 'Utilisez UpdateHostKeys yes ou désactivez ControlPersist dans cette portée.',
      invalidSyntax: 'Ajoutez la valeur requise et fermez tous les guillemets.',
      invalidMatchCondition: 'Utilisez une condition Match prise en charge avec sa valeur requise.',
    },
  },
  es: {
    ...
    allRules: 'Ver las 25 guías de reglas',
    rules: [
      ...
      { code: 'NEGATED_HOST', title: 'Hosts solo negados', text: 'Encuentra bloques Host que nunca pueden coincidir de forma positiva.' },
      { code: 'PROXY_CONFLICT', title: 'Métodos proxy en conflicto', text: 'Muestra cuándo ProxyCommand y ProxyJump compiten en el mismo ámbito.' },
      { code: 'LOCAL_COMMAND_DISABLED', title: 'Comandos locales ignorados', text: 'Avisa cuando LocalCommand existe pero no está habilitado.' },
      { code: 'INVALID_TOKEN', title: 'Tokens de porcentaje no válidos', text: 'Comprueba los marcadores permitidos por cada directiva.' },
    ],
    ...
    browserColumn: 'En este navegador', browserItems: ['Sin instalación', 'Sin subidas', '17 reglas de contenido', 'Móvil y escritorio'],
    ...
    findingMessages: {
      ...
      invalidIntegerRange: "{directive} tiene el valor no válido '{target}'. Se espera un entero entre {min} y {max}.",
      ...
      negatedOnlyHost: 'Host {patterns} no contiene ningún patrón positivo y nunca puede coincidir.',
      proxyConflict: '{ignoredDirective} no tiene efecto porque {firstDirective} ya se definió en la línea {first}.',
      localCommandDisabled: 'LocalCommand se ignora porque PermitLocalCommand no está activado.',
      invalidPercentToken: '{token} no es compatible con {directive}.',
      unknownDirective: '{directive} no es una opción conocida del cliente OpenSSH.',
      deprecatedOption: '{directive} está obsoleta o ya no es compatible.',
      controlPersistUnused: 'ControlPersist no tiene efecto sin ControlMaster.',
      updateHostKeysPersist: 'UpdateHostKeys ask no puede pedir confirmación con ControlPersist activo.',
      invalidSyntax: '{directive} tiene un argumento ausente o vacío, o comillas sin cerrar.',
      invalidMatchCondition: 'La condición Match cerca de {criterion} no es válida.',
    },
    findingHints: {
      ...
      invalidIntegerRange: 'Usa un número entero dentro del rango indicado.',
      ...
      negatedOnlyHost: 'Añade un patrón positivo como * o elimina el bloque.',
      proxyConflict: 'Conserva el único método proxy que debe aplicarse en este ámbito.',
      localCommandDisabled: 'Añade PermitLocalCommand yes en un ámbito aplicable o elimina LocalCommand.',
      invalidPercentToken: 'Usa un token compatible o %% para un signo de porcentaje literal.',
      unknownDirective: 'Comprueba la ortografía o protege una opción específica de plataforma con IgnoreUnknown.',
      deprecatedOption: 'Elimina la opción o sustitúyela por su equivalente actual de OpenSSH.',
      controlPersistUnused: 'Activa ControlMaster en un ámbito aplicable o elimina ControlPersist.',
      updateHostKeysPersist: 'Usa UpdateHostKeys yes o desactiva ControlPersist en este ámbito.',
      invalidSyntax: 'Añade el valor requerido y cierra todas las comillas.',
      invalidMatchCondition: 'Usa una condición Match compatible con el valor requerido.',
    },
  },
};
